...
function reserveViewportForBar(){
  const bar = getBar();
  if (!bar) return;
  const h = Math.ceil(bar.getBoundingClientRect().height || 0);
  document.documentElement.style.setProperty('--bsr-player-reserve', `${Math.max(110, h)}px`);
}

function injectFooterPlayerCSS(){
  if (document.getElementById('bsr-footer-player-mobile-fixes')) return;
  const style = document.createElement('style');
  style.id = 'bsr-footer-player-mobile-fixes';
  style.textContent = `
    :root{
      --bsr-row1-h: 26px;
      --bsr-row2-h: 42px;
      --bsr-safe-h: calc(env(safe-area-inset-bottom) + 10px);
      --bsr-cover: calc(var(--bsr-row1-h) + var(--bsr-row2-h) + 8px);
      --bsr-player-reserve: calc(var(--bsr-row1-h) + var(--bsr-row2-h) + var(--bsr-safe-h) + 28px);
      --bsr-btn: calc(var(--bsr-row2-h) * .72);
    }

    html, body{ overflow-x:hidden; }
    body{ padding-bottom: var(--bsr-player-reserve); }
    ...
    #audio-footer .bsr-shell{
      display:grid;
      grid-template-columns: var(--bsr-cover) minmax(0,1fr) auto;
      grid-template-rows: var(--bsr-row1-h) var(--bsr-row2-h) var(--bsr-safe-h);
      grid-template-areas:
        "cover wave time"
        "cover meta meta"
        "safe safe safe";
    ...
    #audio-footer .bsr-wave-cell{
      grid-area:wave;
      min-width:0;
      align-self:center;
      height:var(--bsr-row1-h);
      overflow:hidden !important;
    }
    #audio-footer .bsr-meta-cell{
      grid-area:meta;
      min-width:0;
      display:flex;
      align-items:center;
    ...
    #audio-footer .cover{
      width:var(--bsr-cover) !important;
      height:var(--bsr-cover) !important;
      min-width:var(--bsr-cover) !important;
      min-height:var(--bsr-cover) !important;
    ...
    #audio-footer .wf.playing svg,
    #audio-footer .wf.playing .wf-svg{
      filter: drop-shadow(0 0 5px rgba(255,138,61,.28));
    }
    #audio-footer .wf svg,
    #audio-footer .wf .wf-svg{
      display:block !important;
      width:100% !important;
      height:100% !important;
      overflow:hidden !important;
    ...
    #audio-footer .btn-ctl{
      width:var(--bsr-btn) !important;
      height:var(--bsr-btn) !important;
      min-width:var(--bsr-btn) !important;
      min-height:var(--bsr-btn) !important;
    ...
    #audio-footer .bt-buf{display:none !important;}

    #audio-footer .queue-panel{
      position:absolute;
      right:12px;
      left:auto;
      bottom: calc(var(--bsr-player-reserve) - var(--bsr-safe-h) + 6px);
    ...
    @media (max-width: 720px) and (orientation: portrait){
      #audio-footer .title,
      #audio-footer .artist{ display:none !important; }
      #audio-footer .cat{ display:block !important; font-size:11px !important; opacity:.92; }
      #audio-footer .bt-ind .lbl{ display:none !important; }
      #audio-footer .bsr-meta-cell{ gap:8px; }
      #audio-footer .queue-panel{
        left:8px;
        right:8px;
        width:auto;
    ...
    @media (min-width: 721px){
      :root{
        ...
        --bsr-safe-h: 12px;
        --bsr-cover: calc(var(--bsr-row1-h) + var(--bsr-row2-h) + 8px);
        --bsr-player-reserve: calc(var(--bsr-row1-h) + var(--bsr-row2-h) + var(--bsr-safe-h) + 32px);
        --bsr-btn: calc(var(--bsr-row2-h) * .68);
      }
      #audio-footer{
        padding: 10px 16px 0;
      }
      #audio-footer .bsr-shell{
        grid-template-columns: var(--bsr-cover) minmax(280px,1fr) auto;
        column-gap:14px;
      }
      #audio-footer .title{font-size:13px !important;}
      #audio-footer .cat,
      #audio-footer .artist,
      #audio-footer .bsr-time-wrap,
      #audio-footer .bt-ind{font-size:11px !important;}
    }
  `;
...
function normalizeFooterLayout(){
  const bar = getBar();
  if (!bar || bar.dataset.bsrLayoutReady === '1') return;

  const cover = bar.querySelector('.cover');
  const wave = bar.querySelector('.wf');
  const title = bar.querySelector('.title');
  const artist = bar.querySelector('.artist');
  const cat = bar.querySelector('.cat');
  const prevBtn = bar.querySelector('.btn-ctl.prev');
  const toggleBtn = bar.querySelector('.btn-ctl.toggle');
  const nextBtn = bar.querySelector('.btn-ctl.next');
  const queueBtn = bar.querySelector('.btn-ctl.queue');
  const btInd = bar.querySelector('.bt-ind');
  const queuePanel = bar.querySelector('.queue-panel');
  const t0 = bar.querySelector('.t0');
  const ttot = bar.querySelector('.ttot');

  const shell = document.createElement('div');
  shell.className = 'bsr-shell';

  const coverCell = document.createElement('div');
  coverCell.className = 'bsr-cell bsr-cover-cell';

  const waveCell = document.createElement('div');
  waveCell.className = 'bsr-cell bsr-wave-cell';

  const metaCell = document.createElement('div');
  metaCell.className = 'bsr-cell bsr-meta-cell';

  const timeCell = document.createElement('div');
  timeCell.className = 'bsr-cell bsr-time-cell';

  const btCell = document.createElement('div');
  btCell.className = 'bsr-cell bsr-bt-cell';

  const safeCell = document.createElement('div');
  safeCell.className = 'bsr-cell bsr-safe-cell';

  const metaText = document.createElement('div');
  metaText.className = 'bsr-meta-text';

  const controls = document.createElement('div');
  controls.className = 'bsr-controls';

  const timeWrap = document.createElement('div');
  timeWrap.className = 'bsr-time-wrap';

  if (cover) coverCell.appendChild(cover);
  if (wave) waveCell.appendChild(wave);

  if (cat) metaText.appendChild(cat);
  if (title) metaText.appendChild(title);
  if (artist) metaText.appendChild(artist);

  [prevBtn, toggleBtn, nextBtn, queueBtn].forEach(btn => {
    if (btn) controls.appendChild(btn);
  });

  metaCell.appendChild(metaText);
  metaCell.appendChild(controls);
  if (btInd) metaCell.appendChild(btInd);

  if (t0 || ttot){
    if (t0) timeWrap.appendChild(t0);
    const sep = document.createElement('span');
    sep.className = 'bsr-time-sep';
    sep.textContent = ' / ';
    timeWrap.appendChild(sep);
    if (ttot) timeWrap.appendChild(ttot);
    timeCell.appendChild(timeWrap);
  }

  shell.appendChild(coverCell);
  shell.appendChild(waveCell);
  shell.appendChild(metaCell);
  shell.appendChild(timeCell);
  shell.appendChild(btCell);
  shell.appendChild(safeCell);

  [...bar.children].forEach(ch => {
    if (queuePanel && ch === queuePanel) return;
    ch.remove();
  });

  bar.appendChild(shell);
  if (queuePanel) bar.appendChild(queuePanel);

  bar.dataset.bsrLayoutReady = '1';
}
...
function updateMetaUI(rel) {
  ...
  requestAnimationFrame(reserveViewportForBar);
}
...
function updateBufferUI(cur, dur){
  const bar = getBar(); if(!bar) return;
  const el = bar.querySelector('.bt-buf');
  if (!el) return;
  el.textContent = '';
  el.style.opacity = '0';
}
...
function ensureQueueUI(){
  ...
  if (!qbtn.dataset.wired){
    qbtn.dataset.wired = '1';
  ...
  if (!document.body.dataset.bsrQueueOutside){
    document.body.dataset.bsrQueueOutside = '1';
    document.addEventListener('click', (evt) => {
      const activeBar = getBar();
      const activePanel = activeBar?.querySelector('.queue-panel');
      const activeBtn = activeBar?.querySelector('.btn-ctl.queue');
      if (!activePanel || activePanel.style.display !== 'block') return;
      if (activePanel.contains(evt.target) || activeBtn?.contains(evt.target)) return;
...
function ensureBtUI(){
  ...
  if (!el){
    ...
    el.className = 'bt-ind';
    el.style.cssText = 'margin-left:10px;font-size:11px;opacity:.7;display:flex;align-items:center;gap:6px;';
    el.innerHTML = `<span class="dot" style="width:8px;height:8px;border-radius:50%;background:rgba(255,255,255,.25);display:inline-block;flex:0 0 auto;"></span>
                    <span class="lbl">BT</span>
                    <span class="bt-buf" style="margin-left:4px;opacity:0;transition:opacity .2s;white-space:nowrap;"></span>`;
    const queueBtn = bar.querySelector('.btn-ctl.queue');
    const toggleRow = queueBtn?.parentNode || bar.querySelector('.right') || bar;
...
function updateBtUI(){
  ...
  const lbl = bar.querySelector('.bt-ind .lbl');
  if (!dot) return;

  const hot = (Date.now() - btLastRemoteTs) < 15000;
  const on = (btDeviceClass === 'controller') || hot;

  dot.style.background = on ? 'rgba(0,255,140,.82)' : 'rgba(255,255,255,.28)';
  dot.style.boxShadow = on ? '0 0 10px rgba(0,255,140,.45)' : 'none';
  if (lbl) lbl.style.opacity = on ? '1' : '.8';
}
...
function buildWavePath(peaks, W=100, H=24){
  const top = [];
  const bot = [];
  const n = peaks.length;
  for (let i=0;i<n;i++){
    const x = (i/(n-1))*W;
    const amp = 2 + peaks[i] * (H * 0.32);
    const yTop = (H/2) - amp;
...
function renderWave(peaks){
  ...
  const uid = ++WF_UID;
  const W = 100, H = Math.max(18, Math.min(28, wf.clientHeight || 24));
  const pathD = buildWavePath(peaks, W, H);

  const accent = getComputedStyle(document.documentElement).getPropertyValue('--accent') || '#ff914d';
...
function wireAudioEvents(a){
  ...
  a.addEventListener('play', () => {
    state.playing = true;
    setToggleUI(true);
    document.querySelector('#audio-footer .wf')?.classList.add('playing');
  ...
  a.addEventListener('pause', () => {
    state.playing = false;
    setToggleUI(false);
    document.querySelector('#audio-footer .wf')?.classList.remove('playing');
    refreshProgressTimer();
    updateMediaSessionState();
  });
...
async function playAt(index) {
  ...
  while (i >= 0 && i < state.queue.length) {
    ...
    if (relTry && srcTry) {
      ...
      renderQueueList();
      requestAnimationFrame(() => { normalizeFooterLayout(); reserveViewportForBar(); });
      return;
    }
    i += step;
  }
...
export function wireFooterControls() {
  ...
  injectFooterPlayerCSS();
  requestAnimationFrame(() => { normalizeFooterLayout(); reserveViewportForBar(); });
  window.addEventListener('resize', reserveViewportForBar, { passive:true });
